...
HOMEASSISTANT_INPUT_BOOLEAN_ENTITY_ID = os.environ.get(
    "HOMEASSISTANT_INPUT_BOOLEAN_ENTITY_ID",
    "input_boolean.input_boolean_volttron_test_toggle",
)

skip_msg = (
    "Set HOMEASSISTANT_ACCESS_TOKEN to run the real Home Assistant input_boolean "
    "integration test."
)

pytestmark = pytest.mark.skipif(
    not (HOMEASSISTANT_TEST_IP and HOMEASSISTANT_ACCESS_TOKEN and HOMEASSISTANT_PORT),
    reason=skip_msg,
)

HOMEASSISTANT_DEVICE_TOPIC = "devices/home_assistant_input_boolean"
...
def volttron_instance():
    ...
    with patch.object(platformwrapper_module, "create_volttron_home", _short_volttron_home):
        wrapper = build_wrapper(
            ...
            ssl_auth=False,
            auth_enabled=False,
            instance_name="ha_input_boolean_test",
        )
        try:
            yield wrapper
        finally:
...
@pytest.fixture(scope="module")
def input_boolean_configured_platform(volttron_instance, platform_driver):
    capabilities = [{"edit_config_store": {"identity": PLATFORM_DRIVER}}]
    volttron_instance.add_capabilities(volttron_instance.dynamic_agent.core.publickey, capabilities)

    registry_config = "homeassistant_input_boolean_test.json"
    registry_obj = [
        {
            "Entity ID": HOMEASSISTANT_INPUT_BOOLEAN_ENTITY_ID,
            "Entity Point": "state",
            ...
            "Starting Value": 0,
            "Type": "int",
            "Notes": "Home Assistant input_boolean integration test",
        }
    ]
    ...
    agent.vip.rpc.call(
        PLATFORM_DRIVER, "set_point", "home_assistant_input_boolean", "bool_state", 0
    ).get(timeout=20)
    gevent.sleep(5)
...
def test_input_boolean_platform_round_trip(input_boolean_configured_platform):
    agent = input_boolean_configured_platform

    off_value = agent.vip.rpc.call(
        PLATFORM_DRIVER, "get_point", "home_assistant_input_boolean", "bool_state"
    ).get(timeout=20)
    assert off_value == "off"

    agent.vip.rpc.call(
        PLATFORM_DRIVER, "set_point", "home_assistant_input_boolean", "bool_state", 1
    ).get(timeout=20)
    gevent.sleep(5)
    ...
    scraped_on = agent.vip.rpc.call(
        PLATFORM_DRIVER, "scrape_all", "home_assistant_input_boolean"
    ).get(timeout=20)
    assert scraped_on == {"bool_state": 1}
    ...
    assert scraped_off == {"bool_state": 0}
